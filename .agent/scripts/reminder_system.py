...
REMINDER_STATE_FILE = Path(".agent/.reminder_state.json")
...
def check_session_reminders():
    ...
    if should_remind_end_of_day(session, state):
        ...
        print("   → python .agent/scripts/auto_session.py end")
        print()
...
def check_completion_reminder():
    ...
    try:
        ...
        if "candidatas a conclusão:" in result.stdout:
            ...
            print(result.stdout)
            print()
            print("💡 Use: python .agent/scripts/auto_finish.py --check-context")
            print()
